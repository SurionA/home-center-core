...
const consoleFormat = printf((params) => {
    ...
    return `${new Date(options.timestamp).toLocaleString()} ${chalk.blueBright(
        labelFormat,
    )} ${colorized(options.level.toUpperCase())}: ${messages}`;
});
...
module.exports = (name) => {
    ...
    container.add(name, {
        format: combine(label({ label: name }), timestamp(), format.json()),
        transports: [
            new transports.Console({
                level: process.env.NODE_ENV === 'DEV' ? 'debug' : 'info',
                format: combine(label({ label: name }), timestamp(), format.splat(), consoleFormat),
            }),
            new transports.File({
